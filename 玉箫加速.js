(function () {
    ...
    _trigger.innerHTML = [
        ...
        '</video>',
        '<img class="xiao-img-static" src="xiao_x_transparent.png" alt="\u7389\u7bab">',
    ...
    function _mountUI() {
        ...
        if (_xiaoVideo) {
            ...
            // ===== 微信兜底：使用静态透明 PNG =====
            // 微信 X5/XWeb 对 SVG filter 应用于 video 的支持存疑，且 mp4(H.264) 无 alpha 通道，
            // 若滤镜失效会显示绿幕。微信环境直接隐藏 video，改用已去底的透明静态图。
            if (/MicroMessenger|wxwork/i.test(navigator.userAgent)) {
                var _staticImg = _trigger.querySelector('.xiao-img-static');
                if (_staticImg) {
                    _xiaoVideo.style.display = 'none';
                    _staticImg.style.display = 'block';
                    try { _xiaoVideo.pause(); } catch (e4) { }
                }
            }
        }
